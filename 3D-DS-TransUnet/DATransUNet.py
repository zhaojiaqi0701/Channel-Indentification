...
# from torch.nn import Module, Sequential, ReLU, AdaptiveMaxPool3d, AdaptiveAvgPool3d, \
...
#         x = self.up(x)
#         if skip is not None:
#             if skip.size(1) == 64 and x.size(1) == 64:
#                 skip = self.da(skip)
#             if skip.size(1) == 256 and x.size(1) == 256:
#                 skip = self.da2(skip)
#             if skip.size(1) == 512 and x.size(1) == 512:
#                 skip = self.da3(skip)
...
# #     net = DA_Transformer(config, img_size=256, num_classes=21843, zero_head=False, vis=False)

# #     input = torch.randn(1, 3, 256, 224, 224)
# #     output = net(input)
# #     print(output.shape)
...
if __name__ == '__main__':
    config = configs.get_r50_b16_config()
    net = DA_Transformer(config, img_size=128, num_classes=21843, zero_head=False, vis=False)
