...
class DANetHead(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv8 = nn.Sequential(nn.Dropout3d(0.05, False), nn.Conv3d(inter_channels, out_channels, 1),
                                   nn.ReLU())
        print(inter_channels,out_channels)
        # self.conv9 = nn.Sequential(nn.Dropout3d(0.05, False), nn.Conv3d(inter_channels, out_channels, 1),
        #                        nn.ReLU())
    def forward(self, x):
        feat1 = self.conv5a(x)
        sa_feat = self.sa(feat1)
        ...
        sasc_output = self.conv8(feat_sum)
        # sasc_output1 = self.conv9(sasc_output)
        return sasc_output
