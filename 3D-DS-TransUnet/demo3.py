...
def plot2(s1, s2, x, slice_idx, cmap='jet', clab="Amplitude", cmin=None, cmax=None, title=None, bottom_title=None, png=None, save_dir="."):
    ...
    plt.show()
# def plot2(s1, s2, x, u=None, g=None, x1=None, c=None, cmap='jet', clab="Amplitude", cmin=None, cmax=None, title=None, bottom_title=None,
#            png=None, save_dir="."):
#     fig, ax = plt.subplots(figsize=(8, 8))
#     mid_slice = x[ :,40, :]
#     mid_slice = mid_slice[0:252, 0:240]
...
#         ax.set_xlabel(title, fontsize=15, labelpad=20)
#         ax.xaxis.set_label_position('bottom')
#         ax.xaxis.set_ticks_position('none')  # 不显示底部的刻度
#     # 添加底部标题
#     if bottom_title:
#         fig.text(0.45, 0.05, bottom_title, ha='center', fontsize=15)
#     # 调整标签字体大小和位置
...
#         if not os.path.exists(save_dir):
#             os.makedirs(save_dir)
#         plt.savefig(os.path.join(save_dir, png + '60-double.png'))
#         plt.savefig(os.path.join(save_dir, png + '-triple-y.png'))
...
def plot21(s1, s2, x, slice_idx, cmap='jet', clab="Amplitude", cmin=None, cmax=None, title=None, bottom_title=None,
    ...
    mid_slice = x[slice_idx, :, :]
    mid_slice = mid_slice[0:256, 0:384]
    im = ax.imshow(mid_slice, cmap=cmap, vmin=cmin, vmax=cmax, aspect='auto')
...
# #     save_dir = './png' 
# #     # plot3(s1, s2, s3, gx, cmin=-2, cmax=2, ks=ks, ae=ae, vt=vt, clab="Amplitude", png="gx")
# #     # plot3(s1, s2, s3, gx, fx, cmin=0.2, cmax=1, ks=ks, ae=ae, vt=vt, cmap=plt.cm.jet, clab="Karst probability", png="fp")
# #     plot2(s1, s2, gx, u=None, g=None, x1=None, c=None, cmap='gray', clab="truth", title=' ',bottom_title='y=40',
# #           png=fname+'-gx',save_dir=save_dir)
# #     plot2(s1, s2, lx, u=None, g=None, x1=None, c=None, cmap='gray', clab="label",  title=' ',bottom_title='y=40',
# #           png=fname+'-lx',save_dir=save_dir)
# #     plot2(s1, s2, fx, u=None, g=None, x1=None, c=None, cmap='gray', clab="Karst probability",title=' ',bottom_title='y=40',
# #           png=fname+'-fp',save_dir=save_dir)
# def goValid(fname):
#     setupForSubset("validation")
#     global pngDir
#     pngDir = getPngDir()
...
#     fx = readImage("px/" + fname)
#     print(gx.shape)
#     save_dir = '/root/autodl-tmp/png2'
#     slice_indices = range(0, gx.shape[0], 5)

#     for slice_idx in slice_indices:
#         plot2(s1, s2, gx, slice_idx, cmap='seismic', clab=" ", title=None, bottom_title=f't={slice_idx}', png=fname+'DA-gx', save_dir=save_dir)
#         plot2(s1, s2, lx, slice_idx, cmap='gray', clab=" ", title=None, bottom_title=f't={slice_idx}', png=fname+'DA-lx', save_dir=save_dir)
#         plot2(s1, s2, fx, slice_idx, cmap='gray', clab=" ", title=None, bottom_title=f't={slice_idx}', png=fname+'DA-fp', save_dir=save_dir)
...
def goJie():
    # gxfile = "/root/autodl-tmp/data/seismic_192-slice4.npy"# input seismic image
    # fpfile = r"/root/autodl-tmp/data/50your_output_file-DA.dat"  # karst probability by cnn
    gxfile = "/root/autodl-tmp/data/MIG_ALLAGC-1.npy"# input seismic image
    fpfile = r"/root/autodl-tmp/data/50MIG-TU.dat"  # karst probability by cnn
    setupForSubset("jie")
    global pngDir
    pngDir = getPngDir()
    s1, s2, s3 = getSamplings()
    ...
    vt = [-0.25, -0.48, 0.0]
    ae = [-45, 50]
    save_dir = '/root/autodl-tmp/pngTU'
    slice_indices = range(0, gx.shape[0], 1)
    # plot3(s1, s2, s3, gx, cmin=-2, cmax=2, ks=ks, ae=ae, vt=vt, clab="Amplitude", png="gx")
